	...
	public void MakeAnOrderWithoutLogin() throws InterruptedException, IOException {
		...
		try {
			...
			if (storeClosed == false) {
				MobileElement changeButton = (MobileElement) wait
						.until(ExpectedConditions.elementToBeClickable(driver.findElementByAccessibilityId("Change")));
				changeButton.click();
			...
			try {
				MobileElement temp_slideupmessage= (MobileElement) driver.findElementById("com.yum.kfc:id/com_braze_inappmessage_slideup_message");
				if(temp_slideupmessage.isDisplayed())
				{
					temp_slideupmessage.click();
			...
			MobileElement cart_Tab = (MobileElement) wait.until(ExpectedConditions.visibilityOf(driver.findElementByXPath(
